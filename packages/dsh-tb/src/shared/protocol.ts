...
export type TaskRecord = {
  ...
  presetId?: string
  /**
   * 准入 ID（选填）：任务通过准入/工单审核时的编号，纯记录用途，
   * 执行时随任务一并发给执行会话供其核对。
   */
  admissionId?: string
  /**
   * 方案链接或路径（选填）：本任务参考方案的 URL 或本地文件路径，
   * 执行时随任务一并发给执行会话，作为其执行的依据/输入。
   */
  solutionRef?: string
  /**
...
/**
 * Validate a task admission id (选填): trimmed, at most 200 chars;
 * empty → undefined.
 * @param raw - the raw input.
 */
export function normalizeAdmissionId(raw: string | undefined): string | undefined {
  const t = (raw ?? '').trim()
  if (t.length > 200) throw new Error('admissionId must be at most 200 characters')
  return t.length === 0 ? undefined : t
}

/**
 * Validate a task solution link or path (选填): trimmed, at most 2000
 * chars; empty → undefined.
 * @param raw - the raw input.
 */
export function normalizeSolutionRef(raw: string | undefined): string | undefined {
  const t = (raw ?? '').trim()
  if (t.length > 2000) throw new Error('solutionRef must be at most 2000 characters')
  return t.length === 0 ? undefined : t
}
