...
* @param raw - the raw input.
*/
function normalizeAdmissionId(raw) {
	const t = (raw ?? "").trim();
...
* @param raw - the raw input.
*/
function normalizeSolutionRef(raw) {
	const t = (raw ?? "").trim();
	if (t.length > 2e3) throw new Error("solutionRef must be at most 2000 characters");
	return t.length === 0 ? void 0 : t;
}
/**
